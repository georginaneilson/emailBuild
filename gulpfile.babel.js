...
gulp.task('build',
  gulp.series(clean, html, sass, images, inline));
...
// Place HTML files into the dist folder
function html() {
  return gulp.src(['src/html/**/*.html', '!src/html/archive/**/*.html'])
    .pipe(gulp.dest('dist'));
}
...
function watch() {
  gulp.watch('src/html/**/*.html').on('all', gulp.series(html, inline, browser.reload));
  gulp.watch(['../scss/**/*.scss', 'src/assets/scss/**/*.scss']).on('all', gulp.series(resethtml, sass, html, inline, browser.reload));
